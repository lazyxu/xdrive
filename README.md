...
Phase 13D7 closes the current **External Sources** scope with an owner-scoped media read API. `GET /api/v1/sources/<source-id>/items` exposes SourceItem state plus connector-neutral media metadata, supports `?state=active|missing`, and uses bounded `limit/offset` pagination. Collection-item reads expose the same metadata surface, so clients can inspect imported media and album membership without connector-specific database access.

With 13A–13D complete, the current Phase 13 contract is: Synology Photos **Push** supports scan-only and sync execution with ignore rules, resumable transfer, stable identity, restart/remount hardening, and non-destructive missing detection; Yike Photos **Pull** supports encrypted credentials, scheduled scan-only and sync execution, resumable streaming, own/shared albums, collection metadata, and media metadata. Both directions reuse the same Source planner and execution-commit protocol. Deliberate non-goals remain source-side delete propagation / `mirror` semantics, Yike-side mutations, guessed Live Photo pairing, and a Synology Photos metadata adapter; those require separate follow-on phases rather than extending Phase 13 implicitly.

Phase 13D-UI1 completes the **Web External Sources UI**. The authenticated Web client has an “外部来源” entry with real Source-backed cards for Synology Photos and Yike Photos, including Push/Pull direction, scan/sync mode, health and credential state, last-run/success timestamps, and latest scanned item/byte totals. Users can inspect run details, create Sources, edit Source configuration and ignore rules, replace/clear Yike Cookie credentials without reading stored plaintext, and request an immediate scan.

Phase 13D-UI2 completes the **Desktop Source IPC** boundary. The renderer does not call the server directly: `xdrive-agent` advertises the `external-sources` capability and exposes owner-authenticated Source list/run/credential status plus create/update/trigger and credential write/delete operations through its protected loopback IPC. Electron main validates inputs and capability support, preload exposes the typed bridge, and credential reads remain status-only; stored Cookie plaintext is never returned to the Desktop renderer.

Phase 13D-UI3 completes the **Desktop External Sources UI**. Desktop now mirrors the Web management flow with an “外部来源” navigation page, Source status cards, read-only run details, immediate-scan actions, Source settings, secure Yike Cookie replacement/clearing, and “+ 添加来源” for Synology Photos Push or Yike Photos Pull. Source creation includes an in-place xDrive folder browser so users choose a target directory without entering raw node IDs. Synology creation keeps NAS-side Push initiation explicit; Yike creation stores its Cookie through the encrypted server credential path.

Manual scan requests use one connector-neutral persisted request. Yike Pull consumes it on the next worker poll and records the resulting run as `manual`. Synology Push keeps the NAS as the connection initiator: DSM Task Scheduler can run `xdrive-source-agent run --due --interval 6h` once per minute as a lightweight control-plane check; a pending manual request overrides the six-hour scan interval and is consumed when the NAS agent actually starts the run. This provides minute-level Web/Desktop triggering without performing a full Photos traversal every minute.
...
The native **Synology Photos source agent** consumes that protocol through `xdrive-source-agent`. It is designed for DSM Task Scheduler, supports Personal Space and Shared Space simultaneously, batches observations, uses Linux device/inode identity so ordinary renames/moves keep the same external identity, and stores xDrive credentials through the existing Secret Service / private-file backend. `setup --mode scan` is the safe default for statistics-only discovery, while `setup --mode sync` enables Push execution. Setup also pins the filesystem identity of each configured Photos root; every run revalidates it before declaring a complete inventory, preventing an unmounted/replaced root or final symlink from turning into a false whole-library `missing` result. Long scans heartbeat the server run lease even when most discovered objects are locally ignored. See `docs/synology-source-agent.md`.
