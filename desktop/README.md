...
- Cloud Files page for cloud-directory browsing, full-cloud path search, quota composition, recycle-bin restore/permanent delete, file-version history/restore, and share-link create/list/revoke;
...
The client packages are unified. On Windows, `xDriveSetup-amd64.exe` installs Electron Desktop, the headless Agent, and `xd` together. On Linux, `xdrive-linux-amd64.deb` contains the same complete client stack. CI builds an unpacked Electron runtime only; the release pipeline embeds that runtime into the single unified installer for each platform.
...
A background launch keeps the main window hidden while the Tray and Agent monitor run. Transfer progress uses a separate Agent long-poll revision stream, so high-frequency byte progress does not churn the general sync/account status monitor. Diagnostics, self-repair, and cloud API access remain behind Electron Main: the renderer never receives the Agent discovery token or Server access/refresh tokens and does not execute platform commands directly. Cloud data models reuse `ui/shared` in the Renderer so Web and Desktop agree on Node/Quota/Version/Share shapes. Desktop first negotiates `GET /v1/hello` and validates the protocol range. If no Agent is available, Desktop starts the installed Core Agent and waits for it to publish Desktop IPC. Unexpected Agent exits are recovered automatically.
