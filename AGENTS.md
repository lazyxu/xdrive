...
- GitHub `.github/workflows/release.yml` and GitLab `infra/ci/gitlab-release.yml` implement the same release contract. A master push or `v*` tag must package the client installers before tests, then pass the full CI artifact gate before release publication. GitHub invokes the reusable publish workflow from the successful CI run so it downloads the exact installer artifacts from that same run. GitLab enforces the same flow with build/package/test/verify/release-build stage ordering and `needs: ... artifacts: true`.
