...
- GitLab Windows jobs keep Go's module cache under `$CI_PROJECT_DIR/.cache/go-mod` so GitLab can archive and restore it. Go marks module-cache content read-only on Windows, so `default.hooks.pre_get_sources_script` must remove the previous worktree copy before GitLab performs Git clean/fetch; the GitLab cache is restored afterward. Do not replace this with a broad `GIT_CLEAN_FLAGS` exclusion for `.cache`, because persistent Shell-runner worktrees must not bypass GitLab cache isolation.
