...
- GitHub `.github/workflows/release.yml` and GitLab `infra/ci/gitlab-release.yml` implement the same release contract. A master push or `v*` tag builds client installers in parallel with ordinary tests; the full gate still requires the ordinary suites plus exact-installer tests before release publication. GitHub invokes the reusable publish workflow from the successful CI run so it downloads the exact installer artifacts from that same run. GitLab keeps the build/package/test/verify/release-build stage model while using DAG `needs` to overlap independent work and exact-artifact dependencies to preserve byte-for-byte publication.
